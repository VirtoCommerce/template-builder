...
export class PreviewService {
    ...
    // add(block: BlockValuesModel, index: number|null, frameId: string) {
    //     this.send('add', { ...block, __index: index }, frameId);
    // }
    ...
    send(type: string, model: any) {
        const element = document.getElementById('preview-frame-component');
        if (element != null) {
            const target = (<HTMLIFrameElement>element).contentWindow;
            if (!!target) {
                const message = { type: type, content: model };
                try {
                    // todo: only in debug mode
                    // console.log('builder->preview:', message)
                    target.postMessage(message, this.appConfig.config.storeUrl);
                } catch (error) {
                    console.error('Preview unavailable. Reason: ', error);
                }
            }
