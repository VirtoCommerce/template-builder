...
export const getCurrentBlockIndex = createSelector(
    getEditorFeatureState,
    getPage,
    (state, page) => typeof state.currentSectionItem === 'string' ? null : page?.content.findIndex(x => x.id === state.currentSectionItem)
);
...
export const getPageTitle = createSelector(
    ...
    (page, props) => {
        ...
        if (page != null && page.settings) {
            return page.settings.title || page.settings.header || '';
        }
        return '';
    }
);
...
export const getEditItemContext = createSelector(
    ...
    getCurrentSectionItem,
    getCurrentTab,
    (state, block, currentTab) => (<ComponentContext>{
