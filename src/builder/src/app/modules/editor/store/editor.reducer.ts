...
const editorReducers = createReducer(
    initialState,
    on(Actions.addPageItem, (state, { block, index }) => {
        const content = state.page.content;
        const page = index === null
            ? { ...state.page, content: [...content.slice(0, index + 1), block, ...content.slice(index + 1)] }
            : { ...state.page, content: [...state.page.content, block] };
        return { ...state, dirty: true, page, currentSectionItem: block.id, showNewBlockSelector: false };
    }),
